    ...
    def __init__(self, root, images_txt, split_txt, train=True, transform=None):
        ...
        with open(images_txt, 'r') as f:
            for line in f:
                ...
                id_to_path[int(image_id)] = rel_path

        id_to_split = {}
        with open(split_txt, 'r') as f:
            for line in f:
                image_id, is_train = line.strip().split()
                id_to_split[int(image_id)] = int(is_train)

        # class 이름을 ImageFolder와 동일하게 정렬해서 label 부여
        classes = sorted(os.listdir(root))
        class_to_idx = {cls_name: i for i, cls_name in enumerate(classes)}

        for image_id, rel_path in id_to_path.items():
            is_train = id_to_split[image_id] == 1
...
if __name__ == '__main__':
    ...
    cub_root    = '../data/images'
    images_txt  = '../data/images.txt'
    split_txt   = '../data/train_test_split.txt'
    ...
    trainset    = CUBDataset(root=cub_root, images_txt=images_txt, split_txt=split_txt, train=True, transform=transform)
    ...
    testset     = CUBDataset(root=cub_root, images_txt=images_txt, split_txt=split_txt, train=False, transform=transform)
