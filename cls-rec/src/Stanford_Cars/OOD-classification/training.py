...
class CarsDataset(Dataset):
    def __init__(self, image_root, anno_mat, train=True, transform=None):
        ...
        data = loadmat(anno_mat, squeeze_me=True, struct_as_record=False)
        annotations = data['annotations']

        # annotation이 하나만 있을 때도 iterable하게 처리
        if not isinstance(annotations, (list, tuple)) and annotations.shape == ():
            annotations = [annotations.item()]

        for anno in annotations:
            fname = str(getattr(anno, 'fname'))
            ...
            if train:
                # Stanford Cars class id는 1~196이므로 CrossEntropyLoss용으로 0~195로 변환
                class_id = int(getattr(anno, 'class'))
                label = class_id - 1
            else:
                # cars_test_annos.mat에는 일반적으로 class가 없음
                # evaluate()에서는 label을 사용하지 않으므로 dummy label로 둠
                label = -1
...
if __name__ == '__main__':
    ...
    E_model     = Encoder(pretrained=False).cuda(local_rank)
    ...
    test_image_root  = '../data/cars_test/cars_test'
    devkit_root      = '../data/car_devkit/devkit'

    train_anno  = os.path.join(devkit_root, 'cars_train_annos.mat')
    test_anno   = os.path.join(devkit_root, 'cars_test_annos.mat')

    transform   = transforms.Compose([transforms.ToTensor(), transforms.Resize(256, antialias=True), transforms.RandomCrop(224), transforms.RandomHorizontalFlip(), transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))])
    trainset    = CarsDataset(image_root=train_image_root, anno_mat=train_anno, train=True, transform=transform)
    ...
    transform   = transforms.Compose([transforms.ToTensor(), transforms.Resize(256, antialias=True), transforms.CenterCrop(224), transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))])
    testset     = CarsDataset(image_root=test_image_root, anno_mat=test_anno, train=False, transform=transform)
